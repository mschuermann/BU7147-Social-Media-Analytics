---
title: "Analysis of Samsung S22 and Apple iPhone 13"
...
This is the analysis for the video submission in the module BU7147 Social Media Analytics of the programme M.Sc. Business Analytics at Trinity Business School. You can also find our code on Github under the following link: <https://github.com/mschuermann/Social-Media-Analytics>

If one of the visualisations does not show labels or similar, please re-run the chunk and it should work.
...
```{r Apple iPhone 13 data, echo=FALSE, message=FALSE, warning=FALSE}
...
library(scales)
#rescale to a scale of 1 to 5
a_tweets_scale_5 <- sentimentr_apple$ave_sentiment %>%
  rescale(to = c(1,5), from = c(-1,1)) %>%
  mean()
...
#rescale to a scale of 1 to 5
s_tweets_scale_5 <- mean(rescale(sentimentr_samsung$ave_sentiment, to = c(1,5), from = c(-1,1)))
```
...
s_bigram_with_sentiment <- s_bigram_counted %>%
  mutate(weightage = sentiment*freq/sum(sentiment*freq)) %>%
...
ggplot(s_bigram_with_sentiment, aes(x=bigrams, y=weightage))+
  ...
  theme(axis.text = element_text(size = 5))+
  xlab("Positive Word") +
  ggtitle("Samsung - Relative Importance of Positive Words")
```
...
s_bigram_with_neg_sentiment <- s_bigram_counted %>%
  mutate(weightage = sentiment*freq/sum(sentiment*freq)) %>%
  filter(!bigrams %in% "phantom") %>% #phantom is not a bad word in this context but the colour of Samsung, so we filtered it out
  arrange(desc(weightage)) %>%
  tail(3)

s_bigram_sentiments <- rbind(s_bigram_with_sentiment, s_bigram_with_neg_sentiment)
ggplot(s_bigram_sentiments, aes(x=bigrams, y=weightage))+
  theme(axis.text = element_text(size = 15))+
  geom_col()+
  xlab("Word") +
  ggtitle("Samsung - Top 3 Positive & Negative Words")



s_plot_neg <- ggplot(s_bigram_with_neg_sentiment, aes(x=bigrams, y=weightage))+
  theme(axis.text = element_text(size = 5))+
  geom_col()+
  xlab("Negative Word") +
  ggtitle("Samsung - Negative Words")

s_plot_pos <- ggplot(s_bigram_with_sentiment, aes(x=bigrams, y=weightage))+
  geom_col()+
  theme(axis.text = element_text(size = 5))+
  xlab("Positive Word") +
  ggtitle("Samsung - Positive Words")

grid.arrange(s_plot_pos, s_plot_neg, ncol=2)
```
...
a_plot_pos <- ggplot(a_bigram_with_sentiment, aes(x=bigrams, y=weightage))+
  theme(axis.text = element_text(size = 5))+
  geom_col()  +
  xlab("Positive Word") +
  ggtitle("Apple - Positive Words")
```
...
a_plot_neg <- ggplot(a_bigram_with_neg_sentiment, aes(x=bigrams, y=weightage))+
    theme(axis.text = element_text(size = 5))+
    geom_col()+
    xlab("Negative Word") +
    ggtitle("Apple - Negative Words")

grid.arrange(a_plot_pos, a_plot_neg, ncol=2)
```
...
s_word_cors %>%
  ...
  filter(!item1 %in% c("liked", "youtube", "g")) %>%
  filter(!item2 %in% c("liked", "youtube", "g")) %>%
...
a_word_cors %>%
  filter(correlation > .50) %>%
  filter(!item1 %in% c("volumes", "jamaican", "apr", "traded", "exchange", "stock", "downgrade", "pixel", "don", "laughing", "loud", "draw", "lucky")) %>%
  filter(!item2 %in% c("volumes", "jamaican", "apr", "traded", "exchange", "stock", "downgrade", "pixel", "don", "laughing", "loud", "draw", "lucky")) %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = correlation), show.legend = FALSE) +
  geom_node_point(color = "lightblue", size = 5) +
...
plot(fit_s)
rect.hclust(fit_s, k = 3, border = "red") # cut tree into 9 clusters
```
...
plot(fit_a)
rect.hclust(fit_a, k = 3,border = "red") # cut tree into 9 clusters
```

The two most talked about models are the iPhone 13 Pro Max and Samsung Galaxy S22 Ultra. All these terms are already mentioned in other parts of our analysis. Anyways, it is interesting that Apple tweets are more talking about phone cases and new Apple [product?] which "just" "can" "get", while Samsung tweets are comparing the new phone series in the first place to Apple products and apparently discuss about which has the best camera.
...
s_corpus <- tm_map(s_corpus, removeWords, c("galaxi", "samsung", "galaxy", "ultra", "\"samsung", "\"galaxy", "iphone", "iphon", "\"iphon", "appl", "pro", "max", "s" ) )
...
# See the topicmodels API documentation for details
...
# Remove the stems associated with our search terms!
a_corpus <- tm_map(a_corpus, removeWords, c("galaxi", "samsung", "galaxy", "ultra", "\"samsung", "\"galaxy", "iphone", "iphon", "\"iphon", "appl", "pro", "max", "s"))
...
The following R code was implemented to collect data from different Amazon websites. ([\<https://martinctc.github.io/blog/vignette-scraping-amazon-reviews-in-r/\>](https://martinctc.github.io/blog/vignette-scraping-amazon-reviews-in-r/){.uri})
...
## Scraping of Amazon Product Reviews
...
### We repeated this step for Amazon UK, US, India for iPhone 13 models and Samsung S22 models.
...
### Comparison of Average Sentiments in Text to Score given by Users
...
samsung.review.stars <- samsung.reviews$review_star %>%
  substr(1,3) %>%
  as.numeric() %>%
  mean() %>%
  round(1)

iphone.review.stars <- iphone.reviews$review_star %>%
  substr(1,3) %>%
  as.numeric() %>%
  mean() %>%
  round(1)
...
compare.stars <- data.table(
  "Metric"= c("Average 5-Star-Score identified from Sentiments", "Average 5-Star-Score given by users", "Average 5-Star-Score identified from Tweets"),
  "Samsung S22"=c(round(afinn.mean.score_s, digits=1), samsung.review.stars, round(s_tweets_scale_5,1)), 
  "iPhone 13" = c(round(afinn.mean.score_a, digits=1), iphone.review.stars, round(s_tweets_scale_5,1)))
compare.stars
```
...
samsung.correlation.terms <- samsung.reviews.text %>%
  ...
  filter(!word %in% c("days", "protector")) %>%
  group_by(word) %>%
  filter(n() >=7)%>%
...
Which words are correlating with each other and what is it related to?

-   **Snapdragon feature** means the [**Qualcomm Snapdragon 8 Gen 1**](https://www.androidauthority.com/qualcomm-snapdragon-8-gen-1-3063350/) is the processor that the S22 uses in the US models. Other countries in the world receive an Samsung S22 models which uses Samsung's own processor [**Qualcomm Snapdragon 8 Gen 1**](https://www.androidauthority.com/qualcomm-snapdragon-8-gen-1-3063350/).

-   **Fingerprint Reader** is Samsung's feature to unlock the phone. In contrast to the iPhone it still has a fingerprint reader additional to the Face ID but it doesn't have a scanner as a button but an ultrasonic screen that identifies the fingerprint.
...
-   **Battery life of Samsung S22** has different batteries in each model. The strongest one is built in the Samsung S22 Plus (15h 16min). The weakest one in the regular Samsung S22 (12h 23min). The S22 Ultra's battery (13h 17min) is actually not that long-lasting as it has been in the prior model Samsung S21 Ultra (14h 43min).

-   **Feels premium** is referring to Samsung's advertisement that the Samsung S22 Ultra has the ["most premium look and feel"](https://news.samsung.com/global/samsung-galaxy-s22-ultra-offers-the-ultimate-and-most-premium-s-series-experience-yet). The word correlations of **quality**, **build** and **app** seems to refer to the same in different words.

-   **Note Series** is the discontinued series of Samsung that has been merged into the S series, e.g. by integrating the pen feature.

-   **Pre-purchase, pro** and **cons** is correlating with **GB** which could mean that people are comparing storage or RAM power before making a purchase decision of Samsung S22.

-   **Protector** is correlating with the word **days**. We could not find any reason for that, neither from the original reviews. Probably because many people commented how many days they are already using the phone as a reference.

-   **Apple**, **iPhone**, **max** and **pro** are correlated because it is different name parts of iPhone's model. A lot of people compare the flagship model to Samsung S22 (as we do here :-)). The **video** quality seems to be highly important for those comparisons as it correlates as well.
...
bigrams.network.df_s <- samsung.reviews.text %>%
  unnest_tokens(bigram, samsung.reviews.text, token = "ngrams", n = 2) %>%
  separate(bigram, c("word1", "word2"), sep = " ") %>%
...
bigrams.network.df_a <-iphone.reviews.text %>%
  unnest_tokens(bigram, iphone.reviews.text, token = "ngrams", n = 2) %>%
  separate(bigram, c("word1", "word2"), sep = " ") %>%
  filter(!word1 %in% stop_words$word & !word2 %in% stop_words$word) %>%
  dplyr::count(word1, word2, sort = TRUE) %>%
  filter(n > 5)
...
```{r Network for Samsung, message=FALSE, warning=FALSE}
...
People talk about Apple much more emotional in both directions and with a fewer focus on exact descriptions of their issues or positive features. This is because Apple products get advertised and reminded more by the focus on the brand while Samsung always is compared by real features. This is something also coming from the ecosystem of Apple devices that work together smoothly. It is an argument for people to stay loyal to Apple products and increases the barrier to switch. Samsung does not promote the ecosystem in the same way, that's why it is a competitive advantage for Apple. Samsung has to work on their communication and connectivity of Samsung devices.

Samsung had the scandal of Game Optimisation System (GOS) app slowing phones and over 10 thousand apps down. This could be used by Apple to advertise that their processor is working at full power and people have to pay a high price for their device (one of the highest negative sentiments in our analysis) but they can - in contrast - use the processor power without disadvantages that they are paying for.

Apple should introduce a Rose Quartz and/or Serenity (or a combination of both) iPhone colour and/or new accessorizes in these colours because people are talking a lot about it and want it. This could be a market gap that they could take advantage of.

The highest advertising focus is on cameras at both product series. People also discuss cameras a lot, comparing both models about that and make purchase decisions accordingly. ["Differences in color [lie mainly in] temperature, depending on the scene. The Galaxy S22 Ultra tends to have a cooler tone, while the ‌iPhone 13 Pro‌ Max is warmer. The S22 Ultra also tends to elevate highlights, and while it can sometimes appear sharper, some may find the images to be a bit too washed out because of it."](https://www.macrumors.com/2022/03/03/galaxy-s22-ultra-vs-iphone-13-pro-max/) The zoom also tends to be in a higher resolution at Samsung S22. Apple has an ultra-wide angle while Samsung's is smaller. Apple introduced the new Cinematic mode, as already mentioned before, and is better in video recording quality. All feedback on the front camera of Apple is really negative while it is really positive on the rear camera. The rear camera is the one advertised by Apple. Therefore, people keep discussing about the pros and cons of each camera and its a tough decision depending on the neck-and-neck race. Definitely, both companies do good on working and improving the camera systems, for Apple, it should be the next move to improve the front camera's quality.

In general, both phones are really highly advanced. "[While a couple of categories like performance and battery life are decisive victories for the iPhone, for the most part, this was a battle of inches](https://www.laptopmag.com/news/galaxy-s22-vs-iphone-13)". Companies should further promote the different aspects that make them unique, e.g. Samsung could advertise more about the S Pen and its compatibility with the tablet and both could put greater emphasis on the different unlock systems, e.g. Samsung has Face Unlocking system and a fingerprint sensor as options.

Apple should stop selling iPhones on Amazon because customers are complaining about the customer service of Amazon because they do not give easy refunds or replacement for the product on the door step even if they promised but the procedures are really long and most likely to avoid the replacement.
