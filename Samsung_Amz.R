...
#Samsung review of Amazon data: Sentiment Analytics

#Set working directory
setwd("/Users/mohit/Desktop/Trinity College/BA- Semester 2/Social Media Analytics/Group Assignment/Final Amazon Dataset/Amazon Samsung Reviews")
#read data
samsung.reviews <- read.csv("Samsung_Amz.csv")
#View(bose.headphones.reviews)
...
word.freq.table<- samsung.reviews.text %>% 
...
bose.correlation.terms <- samsung.reviews.text %>%
...
bigrams.network.df <- samsung.reviews.text %>%
  unnest_tokens(bigram, review_text, token = "ngrams", n = 2) %>%
  separate(bigram, c("word1", "word2"), sep = " ") %>%
