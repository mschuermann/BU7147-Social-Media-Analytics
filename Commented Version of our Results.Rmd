...
ggplot(s_bigram_with_neg_sentiment, aes(x=bigrams, y=-weightage))+
  ...
  geom_col()+
  xlab("Negative Word") +
  ggtitle("Samsung - Relative Importance of Negative Words")
```
...
ggplot(a_bigram_with_sentiment, aes(x=bigrams, y=weightage))+
  ...
  geom_col()  +
  xlab("Positive Word") +
  ggtitle("Apple - Relative Importance of Positive Words")
```
...
a_bigram_with_neg_sentiment <- a_bigram_counted %>%
  mutate(weightage = sentiment*freq/sum(sentiment*freq)) %>%
  filter(!bigrams %in% c("downgrade", "fuck", "shit", "ass")) %>%
  arrange(desc(weightage)) %>%
  tail(3)
